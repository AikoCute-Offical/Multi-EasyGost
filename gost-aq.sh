...
shell_version="1.0.0(fix chặn quốc tế)"
...
function checknew() {
  ...
  echo -n Có nên cập nhật không\(y/n\)\:
  read checknewnum
...
function check_root() {
  [[ $EUID != 0 ]] && echo -e "${Error} Tài khoản không ROOT hiện tại (hoặc không có quyền ROOT) không thể tiếp tục hoạt động, vui lòng thay đổi tài khoản ROOT hoặc sử dụng ${Green_background_prefix}sudo su${Font_color_suffix}. Lệnh nhận được quyền ROOT tạm thời (bạn có thể được nhắc nhập mật khẩu của tài khoản hiện tại sau khi thực hiện)." && exit 1
}
function check_new_ver() {
  ...
  if [[ -z ${ct_new_ver} ]]; then
    ct_new_ver="2.11.2"
    echo -e "${Error} Không tải được phiên bản gost mới nhất, đang tải xuống phiên bản ${ct_new_ver}"
  else
    echo -e "${Info} Phiên bản mới nhất của gost là ${ct_new_ver}"
  fi
}
function check_file() {
...
function read_protocol() {
  ...
  echo -e "Lưu ý: Thường đặt trên máy bay chuyển tuyến nội địa"
  echo -e "-----------------------------------"
  echo -e "[2] Chuyển tiếp lưu lượng truy cập đường hầm được mã hóa"
  echo -e "Mô tả: Nó được sử dụng để chuyển tiếp lưu lượng với mức mã hóa thấp hơn, mức này thường được đặt trên các máy bay chuyển trong nước."
  ...
  echo -e "Mô tả: Đối với lưu lượng được truyền qua mã hóa gost, tùy chọn này được sử dụng để giải mã và chuyển tiếp đến cổng dịch vụ proxy của máy cục bộ hoặc đến các máy từ xa khác"
  echo -e "      Thường được đặt trên các máy nước ngoài được sử dụng để nhận lưu lượng chuyển tuyến"
  echo -e "-----------------------------------"
  echo -e "[4] Cài đặt một cú nhấp chuột của proxy ss/socks5"
  ...
  echo -e "-----------------------------------"
  echo -e "[6] Nâng cao: Chuyển tiếp các nút tự chọn CDN"
  echo -e "Mô tả: Chỉ cần đặt máy chuyển"
  echo -e "-----------------------------------"
...
function read_d_ip() {
  if [ "$flag_a" == "ss" ]; then
    echo -e "------------------------------------------------------------------"
    echo -e "Tôi có thể hỏi mã hóa ss mà bạn muốn đặt không (chỉ cung cấp những mã thường dùng): "
    echo -e "-----------------------------------"
    ...
    while true; do
      echo -e "Tôi có thể hỏi IP hoặc tên miền mà bạn muốn chuyển tiếp lưu lượng mà máy này nhận được từ ${flag_b} không?"
      read -p "Vui lòng nhập: " peer_ip
      echo -e "Tôi có thể hỏi bạn muốn chuyển tiếp lưu lượng truy cập mà máy này nhận được từ ${flag_b} đến ${peer_ip} ở cổng nào của ${peer_ip} không?"
      read -p "Vui lòng nhập: " peer_port
      echo -e "$peer_ip:$peer_port" >>$flag_c.txt
      read -e -p "Có tiếp tục thêm đích？[Y/n]:" addyn
      [[ -z ${addyn} ]] && addyn="y"
      if [[ ${addyn} == [Nn] ]]; then
        echo -e "------------------------------------------------------------------"
        echo -e "$flag_c.txt đã được tạo trong thư mục gốc, bạn có thể chỉnh sửa tệp này bất kỳ lúc nào để sửa đổi thông tin đích và khởi động lại gost để có hiệu lực"
        echo -e "------------------------------------------------------------------"
        break
      else
        echo -e "------------------------------------------------------------------"
    ...
    echo -e "------------------------------------------------------------------"
    echo -e "IP tự chọn để chuyển tiếp lưu lượng mà máy này nhận được từ ${flag_b}:"
    read -p "Vui lòng nhập: " flag_c
    echo -e "Tôi có thể hỏi bạn muốn chuyển tiếp lưu lượng truy cập mà máy này nhận được từ ${flag_b} đến ${flag_c} ở cổng nào của ${flag_c} không?"
    echo -e "[1] 80"
    echo -e "[2] 443"
    ...
    echo -e "------------------------------------------------------------------"
    echo -e "Tôi có thể hỏi bạn muốn chuyển tiếp lưu lượng truy cập mà máy này nhận được từ ${flag_b} bằng IP hoặc tên miền nào không?"
    echo -e "Lưu ý: IP có thể là IP công cộng của [máy từ xa/máy hiện tại] hoặc IP lặp cục bộ của máy (tức là 127.0.0.1)"
    echo -e "Việc điền địa chỉ IP cụ thể phụ thuộc vào IP mà dịch vụ nhận lưu lượng đang nghe (xem: https://github.com/AQSaikato/Multi-EasyGost)"
    if [[ ${is_cert} == [Yy] ]]; then
      echo -e "Lưu ý: Để mở chứng chỉ TLS tùy chỉnh trên máy đích, hãy nhớ điền vào tên miền ${Red_font_prefix} ${Font_color_suffix}"
    fi
    read -p "Vui lòng nhập: " flag_c
  fi
}
function read_d_port() {
  if [ "$flag_a" == "ss" ]; then
    ...
    echo -e "------------------------------------------------------------------"
    echo -e "Bạn muốn chuyển tiếp lưu lượng truy cập mà máy này nhận được từ cổng nào của ${flag_c} đến ${flag_c}?"
    read -p "Vui lòng nhập: " flag_d
    if [[ ${is_cert} == [Yy] ]]; then
...
function encrypt() {
  ...
  echo -e "Lưu ý: Đối với cùng một chuyển tiếp, loại chuyển tiếp và chuyển tiếp hạ cánh phải tương ứng! Tập lệnh này cho phép tcp + udp theo mặc định"
  echo -e "-----------------------------------"
  read -p "Vui lòng chọn loại hình vận tải chuyển tiếp: " numencrypt

  if [ "$numencrypt" == "1" ]; then
    flag_a="encrypttls"
    echo -e "Lưu ý: Chọn Có để bật xác minh chứng chỉ cho chứng chỉ tùy chỉnh đích nhằm đảm bảo bảo mật. Sau đó, máy đích phải điền vào tên miền ${Red_font_prefix} ${Font_color_suffix}"
    read -e -p "Máy đích có bật chứng chỉ TLS tùy chỉnh hay không？[y/n]:" is_cert
...
function enpeer() {
  echo -e "Bạn muốn đặt loại truyền tải cân bằng tải nào?: "
  echo -e "-----------------------------------"
  echo -e "[1] Chuyển tiếp không được mã hóa"
  echo -e "[2] đường hầm tls"
  echo -e "[3] đường hầm ws"
  echo -e "[4] đường hầm wss"
  echo -e "Lưu ý: Đối với cùng một chuyển tiếp, loại chuyển tiếp và chuyển tiếp hạ cánh phải tương ứng! Tập lệnh này mặc định cho cùng một kiểu truyền cho cùng một cấu hình"
  echo -e "Tập lệnh này chỉ hỗ trợ cân bằng tải đơn giản, vui lòng tham khảo tài liệu chính thức để biết chi tiết"
  echo -e "Tài liệu chính thức về cân bằng tải gost：https://docs.ginuerzh.xyz/gost/load-balancing"
  echo -e "-----------------------------------"
...
function cert() {
  ...
  if [ "$numcert" == "1" ]; then
    ...
    echo -e "[2] Ứng dụng API Cloudflare DNS (yêu cầu APIKEY)"
    echo -e "-----------------------------------"
    read -p "Vui lòng chọn phương thức đăng ký chứng chỉ: " certmethod
    if [ "certmethod" == "1" ]; then
      echo -e "Vui lòng xác nhận rằng cổng ${Red_font_prefix} 80 ${Font_color_suffix} cục bộ không bị chiếm dụng, nếu không ứng dụng sẽ không thành công"
      if "$HOME"/.acme.sh/acme.sh --issue -d "${domain}" --standalone -k ec-256 --force; then
        ...
        if "$HOME"/.acme.sh/acme.sh --installcert -d "${domain}" --fullchainpath $HOME/gost_cert/cert.pem --keypath $HOME/gost_cert/key.pem --ecc --force; then
          ...
          echo -e "Không thay đổi tên thư mục chứng chỉ và tên tệp chứng chỉ; sau khi xóa thư mục gost_cert, hãy sử dụng tập lệnh để khởi động lại, nghĩa là chứng chỉ tích hợp sẵn gost sẽ tự động được bật"
          echo -e "-----------------------------------"
        fi
      else
      ...
      read -p "Vui lòng nhập Cloudflare Global API Key：" cfkey
      export CF_Key="${cfkey}"
      ...
      if "$HOME"/.acme.sh/acme.sh --issue --dns dns_cf -d "${domain}" --standalone -k ec-256 --force; then
        ...
        if "$HOME"/.acme.sh/acme.sh --installcert -d "${domain}" --fullchainpath $HOME/gost_cert/cert.pem --keypath $HOME/gost_cert/key.pem --ecc --force; then
          echo -e "Chứng chỉ SSL được định cấu hình thành công và sẽ tự động được gia hạn. Chứng chỉ và khóa nằm trong thư mục ${Red_font_prefix} gost_cert ${Font_color_suffix} trong thư mục người dùng"
          echo -e "Không thay đổi tên thư mục chứng chỉ và tên tệp chứng chỉ; sử dụng tập lệnh để khởi động lại sau khi xóa thư mục gost_cert, tức là bật lại chứng chỉ tích hợp sẵn gost"
          echo -e "-----------------------------------"
        fi
      else
        echo -e "Tạo chứng chỉ SSL không thành công"
...
function show_all_conf() {
  echo -e "                      Cấu hình GOST                        "
  echo -e "--------------------------------------------------------"
  echo -e "số thứ tự | phương thức \ t | cổng cục bộ \ t | địa chỉ đích: cổng đích"
  echo -e "--------------------------------------------------------"
...
cron_restart() {
  echo -e "------------------------------------------------------------------"
  echo -e "Công việc khởi động lại theo lịch trình gost: "
  echo -e "-----------------------------------"
  ...
  if [ "$numcron" == "1" ]; then
    ...
    echo -e "-----------------------------------"
    echo -e "[1] Mọi? giờ khởi động lại"
    echo -e "[2] Hằng ngày? bấm khởi động lại"
    echo -e "-----------------------------------"
    read -p "Xin vui lòng chọn: " numcrontype
    if [ "$numcrontype" == "1" ]; then
      echo -e "-----------------------------------"
      read -p "Mọi? giờ khởi động lại: " cronhr
    ...
    echo -e "Đã hoàn thành xóa tác vụ khởi động lại theo lịch trình！"
  else
    echo "Lỗi, vui lòng thử lại"
    exit
  fi
...
echo && echo -e "                 tập lệnh cấu hình cài đặt một cú nhấp chuột gost "${Red_font_prefix} [${shell_version}] ${Font_color_suffix}"
  ----------- GOST AQ Vietsub -----------
  Đặc tính: (1)Tập lệnh này sử dụng tệp cấu hình systemd và gost để quản lý gost
            (2)Có thể triển khai nhiều quy tắc chuyển tiếp cùng lúc mà không cần sự trợ giúp của các công cụ khác (chẳng hạn như màn hình)
            (3)Việc chuyển tiếp không bị lỗi sau khi máy khởi động lại
  Hàm số: (1)tcp + udp không được mã hóa và chuyển tiếp, (2) máy chuyển tiếp được mã hóa và chuyển tiếp, (3) máy đích được giải mã và chuyển tiếp
  Web chủ bash：https://aqvpn.me/
...
————————————
 ${Green_font_prefix}4.${Font_color_suffix} Bắt đầu gost
 ${Green_font_prefix}5.${Font_color_suffix} Dừng lại gost
...
————————————
 ${Green_font_prefix}10.${Font_color_suffix} Cấu hình khởi động lại theo lịch gost
